...
func (h *FormHandler) Execute(ctx *context.ModalContext) {
	...
	if ok {
		...
		canProceed, outOfHoursWarning, err := logic.ValidatePanelAccess(ctx, panel)
		if err != nil {
			ctx.HandleError(err)
			return
		}
		...
		ctx.Defer()
		_, _ = logic.OpenTicket(ctx.Context, ctx, &panel, panel.Title, formAnswers, outOfHoursWarning)
